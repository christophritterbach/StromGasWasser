...
public abstract class AbstractDBSupportImpl implements DBSupport {
	...
	public void checkConnection(Connection conn) throws RemoteException {
		...
		try {
			...
			// das Ding liefert in getMessage() den kompletten Stacktrace mit, den brauchen
			// wir
			// nicht (das muellt uns nur das Log voll) Also fangen wir sie und werfen eine
			// neue
	...
	/**
	 * Liefert einen verschluesselten Parameter aus der Config-Datei. Wenn der
	 * Parameter noch nicht verschluesselt in der Konfigurationsdatei liegt, wird er
	 * automatisch verschluesselt. Das wird daran festgemacht, wenn der Wert des
	 * Parameters mit "encrypted:" beginnt
	 * 
	 * @param key der Name des Parameters.
	 * @return der entschluesselte Parameter.
	 */
	String getEncrypted(String key) {
		String value = EnergieDBService.SETTINGS.getString(key, null);
		...
		boolean doEncrypt = EnergieDBService.SETTINGS.getBoolean("encrypt", false);
		...
		try {
			if (encrypted) {
				...
				// Wert ist schon verschluesselt - wir entschluesseln ihn und liefern ihn
				// zurueck
			...
			// Wert ist noch nicht verschluesselt. Dann verschluesseln wir ihn bei der
			// Gelegenheit
			...
			EnergieDBService.SETTINGS.setAttribute(key, PREFIX_ENC + Base64.encode(bos.toByteArray()));
