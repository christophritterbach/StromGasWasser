...
public class GasZaehlerControl extends AbstractControl {

	private ZaehlerGas zaehler;
	...
	public GasZaehlerControl(AbstractView view) {
		super(view);
	}

	private ZaehlerGas getZaehlerGas() {
		if (zaehler != null)
			return zaehler;
		zaehler = (ZaehlerGas) getCurrentObject();
		return zaehler;
	}
	...
	public Input getAbleseDatum() throws RemoteException {
		...
		Date datum = getZaehlerGas().getAbleseDatum();
		if (datum == null)
			datum = new Date();
	...
	public Input getAbleseWert() throws RemoteException {
		...
		ableseWert = new DecimalInput(getZaehlerGas().getAbleseWert(), Settings.DECIMALFORMAT);
	...
	public Input getVerbrauch() throws RemoteException {
		...
		verbrauch = new DecimalInput(getZaehlerGas().getVerbrauch(), Settings.DECIMALFORMAT);
	...
	public Input getIstSchaetzung() throws RemoteException {
		...
		istSchaetzung = new CheckboxInput(getZaehlerGas().isSchaetzung());
	...
	public Input getIstZaehlerAusbau() throws RemoteException {
		...
		istZaehlerAusbau = new CheckboxInput(getZaehlerGas().isZaehlerwechselAus());
	...
	public Input getIstZaehlerEinbau() throws RemoteException {
		...
		istZaehlerEinbau = new CheckboxInput(getZaehlerGas().isZaehlerwechselEin());
	...
	public Input getNotiz() throws RemoteException {
		if (notiz != null)
			return notiz;
		notiz = new TextAreaInput(getZaehlerGas().getNotiz());
		notiz.setName("");
		return notiz;
	}
	...
	public void handleStore() {
		try {
			ZaehlerGas zaehl = getZaehlerGas();
