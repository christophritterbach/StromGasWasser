...
public class GasAbschlagControl extends AbstractControl {

	private AbschlagGas abschlag;
	private DateInput abschlagDatum;
	private DecimalInput abschlagBetrag;
	private TextAreaInput notiz;

	public GasAbschlagControl(AbstractView view) {
		super(view);
	}

	private AbschlagGas getAbschlagGas() {
		if (abschlag != null)
			return abschlag;
		abschlag = (AbschlagGas) getCurrentObject();
		return abschlag;
	}
	...
	public Input getAbschlagDatum() throws RemoteException {
		...
		Date datum = getAbschlagGas().getAbschlagDatum();
		if (datum == null)
			datum = new Date();
	...
	public Input getAbschlagBetrag() throws RemoteException {
		...
		abschlagBetrag = new DecimalInput(getAbschlagGas().getAbschlagBetrag(), Settings.DECIMALFORMAT);
	...
	public Input getNotiz() throws RemoteException {
		if (notiz != null)
			return notiz;
		notiz = new TextAreaInput(getAbschlagGas().getNotiz());
		notiz.setName("");
		return notiz;
	}
	...
	public void handleStore() {
		try {
			AbschlagGas abschl = getAbschlagGas();
