...
public class StromZaehlerReiheControl extends AbstractControl {

	private DateInput zaehlerDatumAnfang;
	private DateInput zaehlerDatumEnde;
	private DecimalInput zaehlerVerbrauch;

	public StromZaehlerReiheControl(AbstractView view) {
		super(view);
	}
	...
	public Input getZaehlerVerbrauch() throws RemoteException {
		...
		zaehlerVerbrauch.setComment(Settings.i18n().tr("kWh"));
	...
	public void handleStore() {
		try {
			...
			while (datum.before(endeDatum)) {
				ZaehlerStrom zw = (ZaehlerStrom) Settings.getDBService().createObject(ZaehlerStrom.class, null);
