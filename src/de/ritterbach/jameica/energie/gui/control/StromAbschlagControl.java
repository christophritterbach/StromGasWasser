...
public class StromAbschlagControl extends AbstractControl {

	private AbschlagStrom abschlag;
	private DateInput abschlagDatum;
	private DecimalInput abschlagBetrag;
	private TextAreaInput notiz;

	public StromAbschlagControl(AbstractView view) {
		super(view);
	}

	private AbschlagStrom getAbschlagStrom() {
		if (abschlag != null)
			return abschlag;
		abschlag = (AbschlagStrom) getCurrentObject();
		return abschlag;
	}
	...
	public Input getAbschlagDatum() throws RemoteException {
		...
		Date datum = getAbschlagStrom().getAbschlagDatum();
		if (datum == null)
			datum = new Date();
	...
	public Input getAbschlagBetrag() throws RemoteException {
		...
		abschlagBetrag = new DecimalInput(getAbschlagStrom().getAbschlagBetrag(), Settings.DECIMALFORMAT);
	...
	public Input getNotiz() throws RemoteException {
		if (notiz != null)
			return notiz;
		notiz = new TextAreaInput(getAbschlagStrom().getNotiz());
		notiz.setName("");
		return notiz;
	}
	...
	public void handleStore() {
		try {
			AbschlagStrom abschl = getAbschlagStrom();
