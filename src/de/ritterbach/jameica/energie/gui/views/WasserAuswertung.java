...
public class WasserAuswertung extends AbstractView {
	...
	private KostenWasser kosten = null;
	...
	public void bind() throws Exception {
		...
		DBIterator kostenListe = service.createList(KostenWasser.class);
		...
		while (kostenListe.hasNext()) {
			...
			this.kosten = (KostenWasser) kostenListe.next();
			...
			auswertung.addAll((List<Auswertung>) service.execute(
					...
					+ "FROM abschlag_wasser "
					+ "WHERE abschlag_datum >= ? AND abschlag_datum <= ? "
					+ "UNION "
					...
					+ "FROM zaehler_wasser "
					+ "WHERE ablese_datum >= ? AND ablese_datum <= ? "
					+ "ORDER BY 1, 2, 7",
					new Date[]{this.kosten.getGueltigVon(), this.kosten.getGueltigBis(), this.kosten.getGueltigVon(), this.kosten.getGueltigBis()}
							, this.rse));
