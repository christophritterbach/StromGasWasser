...
public class WasserAuswertung extends AbstractView {
	...
	public void bind() throws Exception {
		GUI.getView().setTitle(Settings.i18n().tr("Wasser Auswertung"));

		DBService service = Settings.getDBService();
		List<Auswertung> auswertung = (List<Auswertung>) service.execute(
				"WITH kosten AS (SELECT id, gueltig_von, gueltig_bis, grundpreis, arbeitspreis FROM kosten_wasser WHERE abgerechnet = 0) "
						+ "SELECT kosten_wasser.gueltig_von AS datum, kosten_wasser.grundpreis, kosten_wasser.arbeitspreis, NULL AS ablese_wert, NULL AS verbrauch, NULL AS schaetzung, 0 AS zaehler_aus, 0 AS zaehler_ein, NULL AS abschlag_betrag, kosten_wasser.rechnungsabschluss "
						+ "FROM kosten_wasser JOIN kosten ON kosten.id = kosten_wasser.id "
						...
						+ "FROM zaehler_wasser JOIN kosten ON ablese_datum >= kosten.gueltig_von AND ablese_datum <= kosten.gueltig_bis "
						+ "UNION SELECT abschlag_datum AS datum, NULL, NULL, NULL, NULL, NULL, 0, 0, abschlag_betrag, 0 "
						+ "FROM abschlag_wasser JOIN kosten ON abschlag_datum >= kosten.gueltig_von AND abschlag_datum <= kosten.gueltig_bis "
						+ "ORDER BY 1, 8",
				null, this.rse);
