...
public class GasAuswertung extends AbstractView {
		...
		public Object extract(ResultSet rs) throws RemoteException, SQLException {
			...
			while (rs.next()) {
				...
				if (!rs.wasNull()) {
					...
					BigDecimal fak = rs.getBigDecimal("faktor");
					if (!rs.wasNull()) {
						faktor = fak;
					}
				...
				if (!rs.wasNull()) {
					...
					BigDecimal ap = verbrauch.multiply(arbeitspreis).multiply(faktor);
	...
	public void bind() throws Exception {
		GUI.getView().setTitle(Settings.i18n().tr("Gas Auswertung"));

		DBService service = Settings.getDBService();
		List<Auswertung> auswertung = (List<Auswertung>) service.execute(
				"WITH kosten AS (SELECT id, gueltig_von, gueltig_bis, grundpreis, arbeitspreis FROM kosten_gas WHERE abgerechnet = 0) "
						+ "SELECT kosten_gas.gueltig_von AS datum, kosten_gas.grundpreis, kosten_gas.arbeitspreis, kosten_gas.faktor, NULL AS ablese_wert, NULL AS verbrauch, NULL AS schaetzung, 0 AS zaehler_aus, 0 AS zaehler_ein, NULL AS abschlag_betrag, kosten_gas.rechnungsabschluss "
						+ "FROM kosten_gas JOIN kosten ON kosten.id = kosten_gas.id "
						+ "UNION SELECT ablese_datum AS datum, NULL, NULL, NULL, ablese_wert, verbrauch, schaetzung, zaehlerwechsel_aus, zaehlerwechsel_ein, NULL AS abschlag_betrag, 0 "
						+ "FROM zaehler_gas JOIN kosten ON ablese_datum >= kosten.gueltig_von AND ablese_datum <= kosten.gueltig_bis "
						+ "UNION SELECT abschlag_datum AS datum, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, abschlag_betrag, 0 "
						+ "FROM abschlag_gas JOIN kosten ON abschlag_datum >= kosten.gueltig_von AND abschlag_datum <= kosten.gueltig_bis "
						+ "ORDER BY 1, 8",
				null, this.rse);
